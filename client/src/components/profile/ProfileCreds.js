...
class ProfileCreds extends Component {
  render() {
    ...
    const expItems = experience.map(exp =>(
      ...
      <h4>{exp.company}</h4>
      <p>
        <Moment format="DD/DD/YYYY">{exp.from}</Moment> -
        {exp.to === null ? (' Now'): <Moment format="DD/DD/YYYY">{exp.to}</Moment>}
      </p>
    ...
    const eduItems = education.map(edu =>(
      ...
      <h4>{edu.school}</h4>
      <p>
        <Moment format="DD/DD/YYYY">{edu.from}</Moment> -
        {edu.to === null ? (' Now'): <Moment format="DD/DD/YYYY">{edu.to}</Moment>}
      </p>
